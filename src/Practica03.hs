...
--Función auxiliar para Ejercicio 2 y algoritmo de Saturación
...
--Función auxiliar para Ejercicio 2 y para algoritmo de saturacion
regresaCabeza :: [a] -> a
...
saturacion f1 = resolvente (construyeListaSinRepeticion construyeSinRepeticion (regresaCabeza (clausulas (fnc f1))) (clausulas (fnc f1)))

--Funcion auxiliar Ejercicio 2. Devuelve todos las resoluciones de la lista de clausulas recibida concatenada con la lista original
...
resolvente (x:xs)
  | hayClausulaVacia (funcionR (x:xs)) = False
  | listasIguales (funcionR xs) xs = True
  | otherwise = resolvente (funcionR (x:xs))

--Función auxiliar Ejercicio 2
listasIguales :: (Eq a) => [a] -> [a] -> Bool
listasIguales [] [] = True
listasIguales [a1] ys = if (contiene a1 ys) then True else False
listasIguales (x:xs) ys = if (contiene x ys) == False then False else listasIguales xs ys

--Función auxiliar Ejercicio 2
contiene :: (Eq a) => a -> [a] -> Bool
contiene _ [] = False
contiene y (x:xs) = if y == x then True else contiene y xs
